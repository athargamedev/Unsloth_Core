...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check destination connectivity and dataset existence."
    )
    parser.add_argument("profile", help="Workspace profile to activate (e.g. prod, dev)")
    parser.add_argument(
        "destination", help="Destination name, type, or reference (e.g. postgres, warehouse)"
    )
    parser.add_argument(
        "dataset_name", nargs="?", default=None, help="Dataset to check (random if omitted)"
    )
