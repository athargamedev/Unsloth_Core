...
```bash
./ucore pipeline data/npcs/specs/my_npc.json --preset fast-3b
```
...
- `spec`: Path to one `data/npcs/specs/*.json` file.
- `--all`: Validate every spec in `data/npcs/specs/`; this is a repository-wide audit and can fail because checked-in draft or incomplete specs exist.
...
```bash
./ucore validate-spec data/npcs/specs/history_guide.json
...
```bash
./ucore generate data/npcs/specs/my_npc.json --technique docs --docs-manifest path/to/curated_corpus.jsonl
```
...
For the Workflow Assistant tool, `data/npcs/specs/workflow_assistant.json` is a docs-backed audit artifact path. This subject is not intended for production Unity NPC export, and its dataset should be used for offline validation or tooling support only.

**W&B convenience preset:**
```bash
./ucore train data/npcs/specs/my_npc.json --preset wandb
```
...
```bash
./ucore train data/npcs/specs/my_npc.json --preset fast-3b --wandb
```
