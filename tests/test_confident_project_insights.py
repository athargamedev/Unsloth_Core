...
def test_build_component_insights_maps_dataset_failures_to_project_root_causes(tmp_path):
    ...
    failures = [
        ...
        },
        {
            ...
            "metric": {
                ...
                "score": 0.3,
                "reason": "too generic and lacks grounded specifics",
            },
        },
    ]
    ...
    insights = build_dataset_quality_insights(
        ...
        artifact_paths={
            ...
            "quality_failures": "quality_failures.json",
        },
    )
    ...
    assert insights["confident_payload"]["identifier"].startswith(
        "dataset-quality:history_guide:grounded"
    )
    assert (
        insights["confident_payload"]["metricCollection"]["name"] == "unsloth-core-dataset-repair"
    )
    ...
    assert (
        case["customColumnKeyValues"]["repair_action"]
        == "tighten identity/refusal contract templates in spec or generator prompt"
    )
...
def test_write_dataset_quality_insights_creates_actionable_artifact(tmp_path):
        ...
        failures=[
            {
                ...
                "metric": {"score": None, "reason": "timeout"},
            }
        ],
    ...
    assert (
        data["recommended_next_actions"][0]["action"]
        == "rerun a smaller semantic gate or fix judge/Ollama availability before changing data"
    )
...
def test_knowledge_retention_failures_route_to_memory_repair():
        ...
        failures=[
            {
                ...
                "metric": {
                    ...
                    "score": 0.1,
                    "reason": "assistant failed to remember user allergy",
                },
            }
        ],
        npc_key="chef_assistant",
    ...
    assert (
        insights["recommended_next_actions"][0]["action"]
        == "add multi-turn memory repair rows and verify user facts are retained across turns"
    )
