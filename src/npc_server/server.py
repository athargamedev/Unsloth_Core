...
class NPCRequest(BaseModel):
    ...
    conversationalContext: list[str] | None = None
...
class ModelLoader:
    ...
    def load_model(self, npc_key: str, technique: str = "ollama"):
        ...
        if not model_path.exists():
            raise HTTPException(
                status_code=404, detail=f"Model not found for {npc_key} with technique {technique}"
            )

        # TODO: Actually load the model using Unsloth/transformers
        # For now, return a mock that simulates model loading
        self.models[model_key] = {"path": model_path, "npc_key": npc_key, "technique": technique}
...
async def npc_generate(request: NPCRequest):
    ...
    try:
        ...
        return NPCResponse(
            output=output,
            metadata={"npc_key": npc_key, "technique": technique, "model_key": model_key},
        )
