...
async def generate_deepeval_dataset(spec_path: str, output_dir: str, num_cases: int = 30):
    ...
    try:
        synthesizer.generate_goldens_from_contexts(
            contexts=[chunks],
            max_goldens_per_context=num_cases
        )
    ...
    print(f"Generated {len(synthesizer.synthetic_goldens)} cases. Formatting for SFT (Unsloth_Core)...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default=str(PROJECT_ROOT / "subjects" / "datasets"), help="Output directory base")
