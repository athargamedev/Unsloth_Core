#!/usr/bin/env python3
"""
_generate_shared.py — Shared NPC Dataset Generation Backend (INTERNAL MODULE)

⚠  NOT a standalone entry point. Use ``./ucore generate-ollama`` instead.

This module provides shared utilities (LLMGroundingVerifier, ConceptExtractor,
ReferenceDocRetriever, generate_dataset function) imported by the production
entry point ``generate_dataset.py`` and ``ollama_orchestrator.py``.

Legacy callers (generators for non-Ollama techniques, template smoke tests) and
tests import the ``generate_dataset()`` function from this module directly.
"""
...
class LLMGroundingVerifier:
    ...
    def verify(
        ...
        prompt = f"""\
Verify whether the NPC RESPONSE below is factually supported by the CONTEXT
and follows the NPC's domain policy.
...
- ALLOWED DOMAIN lists the subjects the NPC may teach.
- FORBIDDEN DOMAIN lists prohibited request intents or recommendations, not words
  that are automatically banned in otherwise valid domain answers.
- Ordinary ingredients, facts, or techniques do not violate a forbidden topic
  unless the response actually promotes, recommends, or meaningfully advances
  that prohibited intent.

1. Are the response's factual claims supported by the CONTEXT?
2. Is the response about an allowed subject?
3. Does it actually promote or recommend a forbidden intent?
4. Does the response stay in character per the NPC's role?
        ...
        for _attempt in range(3):
            ...
            if res:
                try:
                    ...
                    if guardrail:
                        is_valid, reason = guardrail.validate(
                            assistant_response,
                            [grounding],
                            spec,
                            category=category,
...
def generate_dataset(
    ...
    guardrail = generation_profiles.DialogueGuardrail()
