...
def clean_generic_filler(text: str, concept: str = "this topic") -> str:
    ...
    if cleaned.strip() == (text or "").strip() and any(phrase in cleaned.lower() for phrase in ["everything falls into place", "once you understand"]):
...
def build_category_generation_prompt(category: str, concept_str: str, npc_name: str, player_role: str = "player", subject: str = "history", concepts_str: str = "chronology or sources") -> str:
    return {
...
def build_generation_prompt(npc_name: str, system_prompt: str, setting: str, relationship: str, category: str, concept_str: str, category_prompt: str, grounding: str, player_role: str, max_sentences: int, max_chars: int, multi_turn: bool = False, turn_instruction: str = "", json_shape: str = "") -> str:
    prompt = [
