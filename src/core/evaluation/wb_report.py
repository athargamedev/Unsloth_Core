...
def fetch_eval_runs(entity, project, npc_key=None, limit=3, active_only=True):
    ...
    try:
        ...
        try:
            runs = [
                r
                for r in api.runs(f"{entity}/{project}", per_page=limit)
                if "eval" in (r.tags or [])
            ]
...
def generate_report(entity, project, runs, output_path=None):
    ...
    for npc_key, npc_runs in sorted(by_npc.items()):
        ...
        for run in sorted(npc_runs, key=lambda r: r.created_at, reverse=True):
            ...
            lines.append("|--------|-------|")
            lines.append(
                f"| Run ID | [`{run.id}`](https://wandb.ai/{entity}/{project}/runs/{run.id}) |"
            )
            ...
            # W&B Table link
            lines.append(
                f"**W&B Run:** [Open in W&B](https://wandb.ai/{entity}/{project}/runs/{run.id})"
            )
            lines.append("")
...
""")
    ...
    lines.append("---\n")
    lines.append("*Report generated by `wb_report.py` via W&B API*")
