...
def load_strategy_profile(
    profile: str = DEFAULT_PROFILE, *, path: Path = DEFAULT_STRATEGY_PATH
) -> dict[str, Any]:
...
def density_repair_needed(
    feedback_data: dict[str, Any],
    *,
    min_words: int = DENSITY_TARGET_MIN_WORDS,
    max_words: int = DENSITY_TARGET_MAX_WORDS,
) -> dict[str, Any]:
    ...
    """
    candidate_words = float(
        feedback_data.get("avg_candidate_words") or feedback_data.get("candidate_avg_words") or 0
    )
    baseline_words = float(
        feedback_data.get("avg_baseline_words") or feedback_data.get("baseline_avg_words") or 0
    )
    ...
    if candidate_words == 0 and baseline_words == 0:
        ...
        for info in per_concept.values():
            for ex in info.get("examples", []):
                ...
                if wc is not None:
                    cand_vals.append(float(wc))
                wb = ex.get("baseline_words")
                if wb is not None:
                    base_vals.append(float(wb))
        if cand_vals:
            candidate_words = sum(cand_vals) / len(cand_vals)
        if base_vals:
            baseline_words = sum(base_vals) / len(base_vals)
    ...
    too_short_relative = (
        baseline_words > 0 and candidate_words > 0 and candidate_words <= baseline_words * 0.65
    )
...
def classify_feedback_cycle(
    feedback_data: dict[str, Any], *, profile: str = DEFAULT_PROFILE
) -> dict[str, Any]:
    ...
    if all(
        ...
        action = (
            "run_density_repair"
            if density_decision["needed"] and "density_repair_limit_reached" not in flags
            else "continue_bounded_repair"
        )
...
def print_strategy_summary(profile: str = DEFAULT_PROFILE, *, as_json: bool = False) -> None:
    ...
    print(
        f"Quality gate: {gate.get('mode')} / {gate.get('judge_provider')} / cases={gate.get('cases_per_category')}"
    )
    train = summary.get("training") or {}
    print(
        f"Training: {train.get('preset')} r{train.get('lora_r')} alpha{train.get('lora_alpha')} seq={train.get('max_seq_len')}"
    )
    runtime = summary.get("runtime_eval") or {}
    print(
        f"Runtime eval: base_model_required={runtime.get('requires_base_model')} report_html={runtime.get('report_html')}"
    )
