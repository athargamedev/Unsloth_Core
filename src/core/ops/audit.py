...
class ProjectAudit:
    ...
    def check_environment(self):
        ...
        result = {"timestamp": datetime.now().isoformat(), "checks": {}, "score": 0.0}
        ...
        for port, label in ports.items():
            try:
                res = subprocess.run(
                    f"nc -zv 127.0.0.1 {port} 2>&1", shell=True, capture_output=True, timeout=1
                )
    ...
    def audit_npc_state(self):
        ...
        return {"local_state": state, "timestamp": datetime.now().isoformat()}
                ...
                if train_file.exists():
                    try:
                        ...
                        audit["datasets"][f"{npc_key}/{technique_dir.name}"] = {
                            ...
                            "sanitization_ratio": round(clean_lines / train_lines, 2)
                            if train_lines > 0
                            else 0,
                        }
                        ...
                        if clean_lines < 100:
                            audit["issues"].append(
                                f"⚠️  {npc_key}: Only {clean_lines} clean rows (minimum 100 recommended)"
                            )
                        elif clean_lines == 0:
    ...
    def audit_training_outputs(self):
        ...
        for npc_dir in outputs_dir.iterdir():
            ...
            if runs_dir.exists():
                ...
                if run_folders:
                    latest_run = sorted(run_folders, key=lambda x: x.stat().st_mtime, reverse=True)[
                        0
                    ]
    ...
    def report(self, audit_type="env"):
        ...
        if audit_type in ("full", "dataset"):
            ...
            if datasets["datasets"]:
                for dataset, metrics in datasets["datasets"].items():
                    ...
                    print(
                        f"  {ratio_emoji} {dataset}: {metrics['clean_rows']} clean / {metrics['train_rows']} raw ({ratio:.0%})"
                    )
            else:
        ...
        if audit_type == "full":
            ...
            if training["models"]:
                ...
                print("  ℹ️  No training outputs yet")
            ...
            if eval_audit["results"].get("pipeline_state"):
                print("  ✅ pipeline_state.json found")
            else:
                print("  ℹ️  No eval results yet")
