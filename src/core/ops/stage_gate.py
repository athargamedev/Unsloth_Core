...
STAGE_NAMES = [
    "spec", "generate", "sanitize", "dataset_eval",
    "train", "export", "evaluate", "feedback"
]

def compute_checksum(path: Union[str, Path]) -> Optional[str]:
...
def compute_checksums(paths: List[Union[str, Path]]) -> Dict[str, Optional[str]]:
    """Compute checksums for multiple files. Returns {path: sha256}."""
    return {str(p): compute_checksum(p) for p in paths}

def verify_file(path: Union[str, Path], expected_hash: Optional[str] = None) -> bool:
...
def verify_inputs(stage_name: str, input_paths: List[Union[str, Path]]) -> List[str]:
...
def load_state(manifest_path: Union[str, Path]) -> dict:
...
def save_state(manifest_path: Union[str, Path], state: dict) -> None:
...
def record_stage(stage_name: str, output_paths: List[Union[str, Path]], 
                 manifest_path: Union[str, Path], extra: Optional[dict] = None) -> dict:
...
def get_stage_record(stage_name: str, manifest_path: Union[str, Path]) -> Optional[dict]:
...
def is_stage_complete(stage_name: str, manifest_path: Union[str, Path]) -> bool:
