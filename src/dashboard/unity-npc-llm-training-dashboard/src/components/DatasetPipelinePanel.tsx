...
export function DatasetPipelinePanel({ availableCommands, subjects, onTriggerCommand, jobs }: DatasetPipelinePanelProps) {
  ...
  const [sanitizeConfig, setSanitizeConfig] = useState({
    minLength: 10,
    maxSentences: 5,
    strictMode: false,
    ...
    qualityThresholdPass: 70,
    qualityThresholdFlag: 50,
    discardBelowScore: 0,
    noFixMetadata: false,
    dedup: true,
    dedupReport: false,
  ...
  const [showSanitizeOptions, setShowSanitizeOptions] = useState(false);
  ...
  }, [selectedTechnique, ollamaConfig, sanitizeConfig, evalConfig]);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex-1 overflow-auto custom-scrollbar p-4 space-y-6">
        ...
        {/* Advanced Sanitize Options */}
        {hasSelection && (
          <div className="p-3 border border-line bg-surface/50 rounded-sm">
            <button
              onClick={() => setShowSanitizeOptions(!showSanitizeOptions)}
              className="w-full flex items-center justify-between"
            >
              <label className="text-[10px] font-bold text-ink/40 uppercase tracking-wider">
        ...
        {/* Advanced Dataset-Eval Options */}
        {hasSelection && (
          <div className="p-3 border border-line bg-surface/50 rounded-sm">
            <button
              onClick={() => setShowEvalOptions(!showEvalOptions)}
              className="w-full flex items-center justify-between"
            >
              <label className="text-[10px] font-bold text-ink/40 uppercase tracking-wider">
                <FlaskConical className="w-3 h-3 inline mr-1" />
                Advanced Eval Options
              </label>
              {showEvalOptions ? (
                <ChevronUp className="w-3 h-3 text-ink/30" />
              ) : (
